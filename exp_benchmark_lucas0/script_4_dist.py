...
df_x = pd.read_excel(open('/curdir/datasets/lucas0_train.xlsx', 'rb'), sheet_name='0', index_col=None)
df_cg = pd.read_excel(open('/curdir/datasets/lucas0_train.xlsx', 'rb'), sheet_name='1', index_col=0)
...
def get_result(data_x, data_cg, should_std, trial_no, val_w, val_l1, val_l2):
    ## 1
    ...
    try:
        ...
        model = nonlinear_concept.NotearsMLP(
            ...
            mask=mask, w_threshold=val_w, learned_model=None, ## w_threshold=0.3
            metainfo=metainfo
        )
        W_notears, res = nonlinear_concept.notears_nonlinear(
            model, Xflat, lambda1=val_l1, lambda2=val_l2,
    ...
    try:
        ...
        W_notears3 = nonlinear_old.notears_nonlinear(
            model3, Xflat, lambda1=val_l1, lambda2=val_l2, w_threshold=val_w,
            h_tol=1e-4, rho_max=1e+8
        ...
        for ix, (w, l1, l2) in enumerate(list_wl1l2):
            ...
            for trial_no in range(n_trials):
                result_id = get_result.remote(
                    df_x, df_cg, should_std, trial_no, w, l1, l2
                )
            ...
            with open(
                '/curdir/datasets/d_result_' + str(should_std) + '_' + str(ix) + '.pickle', 'wb'
            ) as handle: 
