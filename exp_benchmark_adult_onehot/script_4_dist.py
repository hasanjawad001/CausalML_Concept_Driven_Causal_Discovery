...
df_x = pd.read_csv('/curdir/datasets/adult_processed.csv', header=None)
df_cg = pd.read_excel(open('/curdir/datasets/adult.xlsx', 'rb'), index_col=0)
...
def get_result(data_x, data_cg, should_std, trial_no):
    ...
    try:
        ...
        model = nonlinear_concept.NotearsMLP(
            ...
            mask=mask, w_threshold=0.15, learned_model=None, ## w_threshold=0.3
            metainfo=metainfo
        )
    ...
    try:
        ...
        W_notears3 = nonlinear_old.notears_nonlinear(
            model3, Xflat, lambda1=0.001, lambda2=0.001, w_threshold=0.15,
            h_tol=1e-4, rho_max=1e+8
...
if __name__=='__main__':
    ...
    for should_std in list_should_std:
        ...
        for ix, (w, l1, l2) in enumerate(list_wl1l2):    
            ...
            with open(
                '/curdir/datasets/d_result_' + str(should_std) + '_' + str(ix+3) + '.pickle', 'wb'
            ) as handle: 
